...
class FacturaController extends Controller
{
    public function client($id){
		$cli=Cliente::orderby('id')->where('id',$id)->get()->first();
    ...
    public function factura(Request $request){
		...
		$venta=Venta::orderby('id')->get()->last();
		$detalle_venta=detalle_Ventas::where('id_venta',$venta->id)->get();
		$serie = str_split($venta->serie_venta,4);
		$correlativo = str_split($venta->serie_venta,5);
		//return new Document();
